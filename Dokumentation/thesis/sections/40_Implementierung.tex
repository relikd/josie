...
Wir greifen innerhalb der \josieclass{AudioUnit} auf die Singleton-Instanz der \cocosclass{SimpleAudioEngine}, die alle nötigen Funktionen liefert, zu. So gesehen ist unsere \josieclass{AudioUnit} nur ein Wrapper um im Code der anderen Klassen nicht ständig \cocosclass{SimpleAudioEngine::getInstance()} aufrufen zu müssen, was zu saubererem Code führt. 

Hierbei sei erwähnt dass ganze Musiktitel, wie die Hintergrundmusik, nicht zwingend geladen werden müssen, da diese nicht öfter hintereinander abgespielt werden. 

\paragraph{Abspielen von Soundeffekten und Musik:} Um nun 
